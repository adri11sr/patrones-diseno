...
class QueryBuilder {
  ...
  execute(): string {
    ...
    const fileds = this.fields.length > 0 ? this.fields.join(', ') : '*';
    ...
    const limitCount = this.limitCount ? `LIMIT ${this.limitCount}` : '';

    // Select id, name, email from users where age > 18 and country = 'Cri' order by name ASC limit 10;
    return `Select ${fileds} FROM ${this.table} ${whereClause} ${orderClause} ${limitCount}`;
  }
}
