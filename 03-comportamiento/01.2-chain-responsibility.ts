...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount > 0 && amount <= 1000){
      console.log(`%cSolicitud de compra de ${amount}€ aprovada por el supervisor`, COLORS.green);
      return;
    }

    console.log('%cCantidad superior a 1000€, tiene que aprovarla un Manager. Pasando a un manager', COLORS.red);
    this.next(amount);
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if ( amount <= 5000){
      console.log(`%cSolicitd de compra de ${amount}€ aprovada por el manager`, COLORS.green);
      return;
    }

    console.log('%cCantidad superior a 5000€, tiene que aprovarla el Director. Pasando al Director', COLORS.red);
    this.next(amount);
...
class Director extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    console.log(`%cSolicitd de compra de ${amount}€ aprovada por el Director`, COLORS.green);
